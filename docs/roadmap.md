...
> **Los Sprints 1–2 corresponden al MVP ya entregado en este reto técnico** (`EventService`, `NotificationService`, Frontend "Registrar Evento" — código en `src/`, sin cambios). El resto del roadmap (Sprints 3–12) es el diseño teórico del sistema completo descrito en `architecture.md`: ticketing con alta concurrencia, pagos, emisión de tickets QR, check-in, búsqueda avanzada, notificaciones multicanal e infraestructura Cloud/Híbrida.

Ver [`architecture.md`](./architecture.md) para el diseño técnico detrás de cada épica, y [`.specify/memory/constitution.md`](../.specify/memory/constitution.md) para las decisiones que ya rigen la Fase 1.
...
2. MVP Core (Eventos) — ya entregado
...
| MVP Core (Eventos) — ya entregado | 1–2 | Implementar `EventService` + `NotificationService`: creación/consulta de eventos, Outbox transaccional, consumo idempotente, reintentos + DLQ | Backend (x2), FullStack | — | DEV → QA |
| MVP Core (Eventos) — ya entregado | 2 | Construir el formulario "Registrar Evento" (Frontend) con validaciones inline y estados de carga/error | Frontend (x2), UI, UX | — | DEV → QA |
| MVP Core (Eventos) — ya entregado | 2 | Suite de pruebas automatizadas (dominio, aplicación, integración, Frontend) y checklist de aceptación del MVP | QA (x2), LT | — | QA |
...
| Pagos | 8 | Integrar `PaymentService` con el PSP externo (tokenización en el cliente, sin tocar datos de tarjeta en el backend) | Backend, LT | **Seguridad** (cumplimiento de datos de pago, alcance PCI-DSS) | DEV |
