...
- **Decision**: Validación JWT Bearer HMAC-SHA256 nativa de ASP.NET Core (`AddAuthentication().AddJwtBearer(...)`), secreto vía `Jwt__SigningKey` (variable de entorno). El token de demo se genera con un pequeño script de consola (`tools/generate-demo-token` o snippet documentado en el README) que firma un JWT con `sub`, `role: Admin|User` y `exp`, usando el mismo secreto de `.env`. Ese token se expone al Frontend vía la variable de entorno `VITE_ADMIN_TOKEN` (nunca como constante hardcodeada en `config.ts`, ver Constitución §4 sobre prohibición de secretos hardcodeados), pasada por `docker-compose.yml` al contenedor del Frontend.
- **Rationale**: No existe emisor real ni login en el MVP (Constitución §5.1); generar el token offline con el mismo secreto configurado es la única forma de que `EventService` lo acepte sin introducir un IdP. Usar una variable de entorno en vez de un literal en el código permite rotarlo sin reconstruir la imagen del Frontend (el `Dockerfile` corre `npm run dev`, que lee `import.meta.env` en caliente) y evita mezclar código con configuración.
...
## 8b. Notificación por correo (simulado por defecto, SMTP real opcional)

- **Decision**: `MailKitEmailSender` arma siempre un `MimeMessage` real con MailKit/MimeKit. Si `Smtp:Host` no está configurado (caso por defecto en `docker compose up` sin variables `SMTP_*` en `.env`), el mensaje solo se loguea (simulación, comportamiento histórico). Si `Smtp:Host` está configurado, se conecta de verdad vía `MailKit.Net.Smtp.SmtpClient` (STARTTLS) al servidor indicado, se autentica si hay `Smtp:Username`/`Smtp:Password`, y envía el correo real a `Smtp:ToAddress`.
- **Rationale**: El enunciado original pide explícitamente "enviar un correo con detalles del evento creado", no "simular" — la simulación por defecto queda como una concesión pragmática para que el MVP funcione sin que cada evaluador deba configurar credenciales de correo reales (mantiene FR-017/SC-008 intactos), pero la capacidad de envío real queda disponible con solo completar variables de entorno, sin tocar código, para quien quiera validar el flujo de punta a punta contra su propia casilla.
- **Alternatives considered**: Exigir SMTP real siempre (sin fallback) — descartado porque rompería el arranque "listo para usar" para cualquiera que clone el repo sin credenciales de correo a mano. Un servicio de sandbox de correo (Mailtrap/Ethereal) prefijado en el código — descartado por atar el proyecto a un proveedor de terceros específico cuando cualquier SMTP estándar (Gmail, Outlook, un sandbox propio) ya funciona vía configuración.
- **Manejo de fallos**: una falla real de conexión/autenticación/envío SMTP se deja propagar sin capturar — así reutiliza el mismo mecanismo de reintentos/DLQ de MassTransit (§2) ya exigido por el reto, en vez de tragarse el error silenciosamente.
- **Configuración**: variables `SMTP_HOST`, `SMTP_PORT`, `SMTP_USERNAME`, `SMTP_PASSWORD`, `SMTP_FROM_ADDRESS`, `SMTP_FROM_NAME`, `SMTP_TO_ADDRESS` en `.env` (todas opcionales; `SMTP_HOST` vacío = modo simulado). Como con `JWT_SIGNING_KEY`, `SMTP_PASSWORD` es un secreto real (Constitución §4) y nunca se hardcodea ni se versiona.
